...
type
  ...
  TfraCameraCapture1 = class(TFrame)
...
constructor TfraCameraCapture1.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TfraCameraCapture1.Destroy;
begin
...
procedure TfraCameraCapture1.SetInitialDir(const Dir: string);
begin
  OpenDialogPictures.InitialDir := Dir;
  SaveDialogPictures.InitialDir := Dir;
end;

function TfraCameraCapture1.GetInitialDir: string;
begin
  result := OpenDialogPictures.InitialDir;
end;

procedure TfraCameraCapture1.StartCapture(OnPhotoCaptured: TOnPhotoCaptured);
begin
...
{$IFDEF MSWINDOWS}
procedure TfraCameraCapture1.StartCaptureOnWindows;
begin
...
procedure TfraCameraCapture1.StopCapture;
begin
  CameraComponent.Active := false;
...
function TfraCameraCapture1.Bitmap: TBitmap;
begin
  fCSForCamAccess.Acquire;
  try
...
procedure TfraCameraCapture1.btnRestartClick(Sender: TObject);
begin
  StartCapture;
end;

procedure TfraCameraCapture1.btnTakeClick(Sender: TObject);
begin
  if assigned(fOnPhotoCaptured) then
    fOnPhotoCaptured(Bitmap, GetFileName);
end;

procedure TfraCameraCapture1.btnTakePhotoFromCameraClick(Sender: TObject);
begin
  StopCapture;
end;

procedure TfraCameraCapture1.CameraComponentSampleBufferReady(Sender: TObject;
  const ATime: TMediaTime);
begin
...
procedure TfraCameraCapture1.DisplayCameraPreviewFrame;
begin
  fCSForCamAccess.Acquire;
  try
    imgCameraPreview.Bitmap.Assign(fBitmapFromCam);
...
procedure TfraCameraCapture1.StartTakePhotoFromLib(
...
function TfraCameraCapture1.GetFileName: string;
var
  dt: string;
begin
...
{$REGION 'Platform specific stuff'}
procedure TfraCameraCapture1.ToastMsg(const Msg: string);
begin
  {$IFDEF MSWINDOWS}
...
{$IFDEF ANDROID}
procedure TfraCameraCapture1.StartCaptureOnAndroid;
var
...
procedure TfraCameraCapture1.RequestPermissionsResultEvent(Sender: TObject;
  const APermissions: TClassicStringDynArray;
  const AGrantResults: TClassicPermissionStatusDynArray);
begin
  // nothing to do
...
procedure TfraCameraCapture1.StartTakePhotoFromLibOnAndroid;
var
...
procedure TfraCameraCapture1.DoDidFinishTakePhotoOnAndroid(Image: TBitmap);
begin
  if assigned(fOnPhotoCaptured) then
...
{$IFDEF MSWINDOWS}
procedure TfraCameraCapture1.StartTakePhotoFromLibOnWindows;
var
  Bitmap: TBitmap;
begin
