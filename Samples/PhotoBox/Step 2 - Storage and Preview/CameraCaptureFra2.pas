...
type
  ...
  TfraCameraCapture2 = class(TFrame)
...
constructor TfraCameraCapture2.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TfraCameraCapture2.Destroy;
begin
...
procedure TfraCameraCapture2.SetInitialDir(const Dir: string);
begin
  OpenDialogPictures.InitialDir := Dir;
  SaveDialogPictures.InitialDir := Dir;
end;

function TfraCameraCapture2.GetInitialDir: string;
begin
  result := OpenDialogPictures.InitialDir;
end;

procedure TfraCameraCapture2.StartCapture(OnPhotoCaptured: TOnPhotoCaptured);
begin
...
{$IFDEF MSWINDOWS}
procedure TfraCameraCapture2.StartCaptureOnWindows;
begin
...
procedure TfraCameraCapture2.StopCapture;
begin
  CameraComponent.Active := false;
...
function TfraCameraCapture2.Bitmap: TBitmap;
begin
  fCSForCamAccess.Acquire;
  try
...
procedure TfraCameraCapture2.btnRestartClick(Sender: TObject);
begin
  StartCapture;
end;

procedure TfraCameraCapture2.btnTakeClick(Sender: TObject);
begin
  if assigned(fOnPhotoCaptured) then
    fOnPhotoCaptured(Bitmap, GetFileName);
end;

procedure TfraCameraCapture2.btnTakePhotoFromCameraClick(Sender: TObject);
begin
  StopCapture;
end;

procedure TfraCameraCapture2.CameraComponentSampleBufferReady(Sender: TObject;
  const ATime: TMediaTime);
begin
...
procedure TfraCameraCapture2.DisplayCameraPreviewFrame;
begin
  fCSForCamAccess.Acquire;
  try
    imgCameraPreview.Bitmap.Assign(fBitmapFromCam);
...
procedure TfraCameraCapture2.StartTakePhotoFromLib(
...
procedure TfraCameraCapture2.SaveImageToFile(Image: TStream;
...
function TfraCameraCapture2.GetFileName: string;
var
  dt: string;
begin
...
{$REGION 'Platform specific stuff'}
procedure TfraCameraCapture2.ToastMsg(const Msg: string);
begin
  {$IFDEF MSWINDOWS}
...
{$IFDEF ANDROID}
procedure TfraCameraCapture2.StartCaptureOnAndroid;
var
...
procedure TfraCameraCapture2.RequestPermissionsResultEvent(Sender: TObject;
  const APermissions: TClassicStringDynArray;
  const AGrantResults: TClassicPermissionStatusDynArray);
begin
  // nothing to do
...
procedure TfraCameraCapture2.StartTakePhotoFromLibOnAndroid;
var
...
procedure TfraCameraCapture2.DoDidFinishTakePhotoOnAndroid(Image: TBitmap);
begin
  if assigned(fOnPhotoCaptured) then
...
{$IFDEF MSWINDOWS}
procedure TfraCameraCapture2.StartTakePhotoFromLibOnWindows;
var
  Bitmap: TBitmap;
begin
...
procedure TfraCameraCapture2.SaveImageToFileOnWindows(Img: TStream;
  const ContentType: string);
var
  Ext: string;
